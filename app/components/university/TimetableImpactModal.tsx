...
export default function TimetableImpactModal({
  ...
  const getSeverityBadge = (severity?: string) => {
    switch (severity?.toUpperCase()) {
      ...
      case 'HIGH':
        return {
          label: 'HIGH IMPACT',
          bg: 'bg-orange-50 dark:bg-orange-500/20 text-orange-700 dark:text-orange-300 border-orange-200 dark:border-orange-500/30',
          dot: 'bg-orange-500',
        };
      case 'MEDIUM':
        return {
          label: 'MEDIUM IMPACT',
          bg: 'bg-amber-50 dark:bg-amber-500/20 text-amber-800 dark:text-amber-300 border-amber-200 dark:border-amber-500/30',
          dot: 'bg-amber-500',
        };
      case 'LOW':
      default:
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/75 backdrop-blur-sm animate-in fade-in duration-200">
      <div
        className="relative w-full max-w-3xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-2xl shadow-slate-900/20 dark:shadow-2xl overflow-hidden flex flex-col max-h-[90vh]"
      ...
      >
        ...
        <div className="p-6 border-b border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950/50 flex items-start justify-between gap-4">
          <div>
            <div className="flex items-center gap-3">
              <span className="px-2.5 py-0.5 text-xs font-semibold uppercase tracking-wider rounded-md bg-indigo-50 dark:bg-indigo-500/20 text-indigo-700 dark:text-indigo-300 border border-indigo-200 dark:border-indigo-500/30">
            ...
            <>
              ...
              {staleError && (
                <div className="p-4 bg-amber-50 dark:bg-amber-950/40 border border-amber-300 dark:border-amber-600/50 rounded-xl text-amber-900 dark:text-amber-200 text-sm space-y-2">
                  ...
                  {onRefresh && (
                    <button
                      onClick={onRefresh}
                      className="px-3 py-1.5 bg-amber-600 hover:bg-amber-500 text-white rounded-lg text-xs font-semibold transition-colors cursor-pointer"
                    >
              ...
              {isBlocked && (
                <div className="p-4 bg-rose-50 dark:bg-rose-950/40 border border-rose-300 dark:border-rose-600/60 rounded-xl text-rose-900 dark:text-rose-200 text-sm space-y-2">
                  ...
                  <ul className="list-disc list-inside text-xs space-y-1 text-rose-800 dark:text-rose-300 font-medium">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 bg-slate-50 dark:bg-slate-950/60 p-4 rounded-xl border border-slate-200 dark:border-slate-800/80">
                {/* BEFORE */}
                <div className="space-y-2 border-b md:border-b-0 md:border-r border-slate-200 dark:border-slate-800 pb-3 md:pb-0 md:pr-4">
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400">Current Schedule (Before)</span>
                    <span className="text-xs px-2 py-0.5 rounded bg-slate-200 dark:bg-slate-800 text-slate-700 dark:text-slate-300 font-medium">Saved</span>
                  </div>
                ...
                <div className="space-y-2 md:pl-2">
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-semibold uppercase tracking-wider text-indigo-600 dark:text-indigo-400">Proposed Change (After)</span>
                    <span className="text-xs px-2 py-0.5 rounded bg-indigo-50 dark:bg-indigo-500/20 text-indigo-700 dark:text-indigo-300 border border-indigo-200 dark:border-indigo-500/30 font-medium">
                      Pending
                    </span>
                  </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                <div className="p-3 bg-slate-50 dark:bg-slate-950/40 border border-slate-200 dark:border-slate-800 rounded-xl">
                  <p className="text-[11px] font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Students Affected</p>
                  <p className="text-2xl font-bold text-slate-900 dark:text-white mt-0.5">{summary?.students_affected ?? 0}</p>
                  <p className="text-[10px] text-slate-500 mt-0.5">Enrolled in section</p>
                </div>

                <div className="p-3 bg-slate-50 dark:bg-slate-950/40 border border-slate-200 dark:border-slate-800 rounded-xl">
                  <p className="text-[11px] font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">New Conflicts</p>
                  <p
                    className={`text-2xl font-bold mt-0.5 ${
                      (summary?.new_conflicts ?? 0) > 0 ? 'text-orange-600 dark:text-orange-400' : 'text-emerald-600 dark:text-emerald-400'
                    }`}
                  >
                ...
                <div className="p-3 bg-slate-50 dark:bg-slate-950/40 border border-slate-200 dark:border-slate-800 rounded-xl">
                  <p className="text-[11px] font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Resolved Conflicts</p>
                  <p className="text-2xl font-bold text-emerald-600 dark:text-emerald-400 mt-0.5">{summary?.resolved_conflicts ?? 0}</p>
                  <p className="text-[10px] text-slate-500 mt-0.5">Cleared by move</p>
                </div>

                <div className="p-3 bg-slate-50 dark:bg-slate-950/40 border border-slate-200 dark:border-slate-800 rounded-xl">
                  <p className="text-[11px] font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Room & Faculty</p>
                  <p
              ...
              {activeTab === 'students' && (
                <div className="space-y-2">
                  {filteredStudents.length === 0 ? (
                    ...
                    <div className="divide-y divide-slate-200 dark:divide-slate-800/80 max-h-56 overflow-y-auto pr-1">
                      {filteredStudents.map((s, idx) => (
                        <div key={idx} className="py-2.5 flex items-center justify-between text-xs">
                          <div className="flex items-center gap-2">
                            <span className="w-6 h-6 rounded-full bg-slate-200 dark:bg-slate-800 text-slate-700 dark:text-slate-300 flex items-center justify-center text-[10px] font-semibold">
                              {s.student_id}
                            </span>
                            <div>
                              <p className="font-semibold text-slate-900 dark:text-white">{s.student_name}</p>
                              <p className="text-slate-500 dark:text-slate-400 text-[11px]">{s.conflict_description}</p>
                            </div>
                          </div>
                          ...
                          <div className="text-right">
                            {s.conflict_type === 'none' ? (
                              ...
                              <span className="px-2 py-0.5 rounded-full text-[10px] bg-amber-50 dark:bg-amber-500/20 text-amber-800 dark:text-amber-300 border border-amber-200 dark:border-amber-500/30 font-medium">
                                Blackout {s.overlap_time ? `(${s.overlap_time})` : ''}
                              </span>
                            ) : (
                              <span className="px-2 py-0.5 rounded-full text-[10px] bg-indigo-50 dark:bg-indigo-500/20 text-indigo-700 dark:text-indigo-300 border border-indigo-200 dark:border-indigo-500/30 font-medium">
                ...
                <div className="space-y-4 p-2 text-xs">
                  <div className="grid grid-cols-2 gap-4">
                    <div className="p-3 bg-slate-50 dark:bg-slate-950/40 border border-slate-200 dark:border-slate-800 rounded-xl space-y-1">
                      <p className="text-slate-500 dark:text-slate-400 uppercase font-semibold text-[10px]">Assigned Room</p>
                      <p className="text-sm font-bold text-slate-900 dark:text-white">{after?.room_label || 'None assigned'}</p>
                      <p className="text-slate-600 dark:text-slate-400">
                        Room Capacity: <span className="text-slate-900 dark:text-white font-semibold">{after?.room_capacity ?? 'N/A'}</span>
                      </p>
                    </div>
                    <div className="p-3 bg-slate-50 dark:bg-slate-950/40 border border-slate-200 dark:border-slate-800 rounded-xl space-y-1">
                      <p className="text-slate-500 dark:text-slate-400 uppercase font-semibold text-[10px]">Section Enrollment</p>
                  ...
                  {summary?.room_issues && summary.room_issues.length > 0 && (
                    <div className="p-3 bg-rose-50 dark:bg-rose-950/30 border border-rose-200 dark:border-rose-800/40 rounded-xl text-rose-800 dark:text-rose-300 space-y-1">
              ...
              {activeTab === 'faculty' && (
                <div className="space-y-3 p-2 text-xs">
                  <div className="p-3 bg-slate-50 dark:bg-slate-950/40 border border-slate-200 dark:border-slate-800 rounded-xl space-y-1">
                    <p className="text-slate-500 dark:text-slate-400 uppercase font-semibold text-[10px]">Assigned Faculty</p>
                    <p className="text-sm font-bold text-slate-900 dark:text-white">{after?.faculty_name || 'No instructor assigned'}</p>
                    <p className="text-slate-600 dark:text-slate-400">
                      Status:{' '}
        ...
        <div className="p-4 border-t border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950/60 flex items-center justify-between gap-3">
          <div className="text-xs text-slate-500 dark:text-slate-400">
            {isBlocked ? (
              <span className="text-rose-600 dark:text-rose-400 font-medium">Resolving blocking constraints is required to apply.</span>
            ) : (
          ...
          <div className="flex items-center gap-2 shrink-0">
            <button
              onClick={onCancel}
              disabled={isApplying}
              className="px-4 py-2 bg-slate-200 hover:bg-slate-300 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 rounded-xl text-xs font-semibold transition-colors disabled:opacity-50 cursor-pointer"
            >
              Cancel
            </button>
            ...
            {!isBlocked && (
              <button
                ...
                className="px-5 py-2 bg-gradient-to-r from-indigo-500 to-indigo-600 hover:from-indigo-600 hover:to-indigo-700 text-white rounded-xl text-xs font-semibold shadow-md hover:shadow-indigo-500/25 transition-all flex items-center gap-1.5 disabled:opacity-50 cursor-pointer"
              >
                {isApplying ? (
                  <>
