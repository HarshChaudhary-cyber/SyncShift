...
function DashboardContent() {
  ...
  const handleCloseBlockModal = () => {
    ...
    // Re-fetch dashboard after modal action
    fetchDashboard(true);
  };

  const handleCloseImportModal = () => {
    setImportModalOpen(false);
    // Re-fetch dashboard after import action
    fetchDashboard(true);
  };
  ...
  if (error && !dashboardData && !loading) {
    return (
      <div className="min-h-screen bg-[var(--bg-primary)] text-[var(--text-primary)] flex flex-col">
        <Navbar onImportClick={handleOpenImport} />
  ...
  return (
    <div className="min-h-screen bg-[var(--bg-primary)] text-[var(--text-primary)] flex flex-col selection:bg-indigo-500/30 selection:text-indigo-200">
      <Navbar onImportClick={handleOpenImport} />

      <main className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 py-6 sm:py-8 space-y-6">
        {/* 1. Greeting Header + current date */}
        <GreetingHeader
          user={dashboardData?.user || (user as any)}
          currentDate={dashboardData?.today?.date}
        />

        {/* 1b. Adaptive Guidance (new user, academic only) */}
        <AdaptiveBanner
        ...
        {/* 2. Immediate Attention (conflicts, limit alerts, or 'You're on track') */}
        <AlertsBanner
        ...
        {/* 4b. Student Academic Summary (Task N3) */}
